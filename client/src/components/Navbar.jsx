...
function Navbar() {
    ...
    const [showCreditPopup, setShowCreditPopup] = useState(false) //to show popup after clicking the credit button on the navbar
    const [showUserPopup, setShowUserPopup] = useState(false)//same like former but for the user icon
    ...
    const handleLogout = async() => {
      try{
         ...
         console.log(`error in logout handling in navbar: ${error}`)  
      }
    }
              ...
              {showUserPopup && (
                <div className='absolute right-0 mt-3 w-48 bg-white shadow-xl border border-gray-200 rounded-xl p-4 z-50'>
                  ...
                  <button 
                   ...
                   className='w-full text-left text-sm py-2 hover:text-black text-gray-600'>Interview History
                  </button>
                  <button 
                   onClick={handleLogout}
                   className='w-full text-left text-sm py-2 flex items-center gap-2 text-red-500'><HiOutlineLogout size={16}/> Logout 
                  </button>
